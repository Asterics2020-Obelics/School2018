...
\begin{frame}{Typical single-branch workflow}
  \begin{enumerate}
    \item Create or clone repository : \texttt{git init}, \texttt{git clone <url>}
    \item Work
      \begin{enumerate}
...
\begin{frame}{\texttt{git add}, \texttt{git mv}, \texttt{git rm}, \texttt{git reset}}
  \begin{tabu}{>{\ttfamily}l X[,L]}
    ...
    git reset \textit{file} & Removes changes/files from the staging area
  \end{tabu}
\end{frame}
...
\begin{frame}[t]{Working using multiple branches – GitHub Workflow}
  \begin{itemize}
    \item There are multiple models of working together with git using branches
    \item Simplest and most popular: \enquote{GitHub-Workflow}
